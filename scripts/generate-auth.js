...
const { GOOGLE_ACCOUNT_USER, GOOGLE_ACCOUNT_PASSWORD } = process.env;
...
(async () => {
  ...
  if (await page.isVisible('input[type="email"]')) {
    if (GOOGLE_ACCOUNT_USER) {
      ...
      console.log(
        "Please type your Google email in the browser and click “Next”…",
      );
    }
  }
  ...
  await page.waitForSelector(
    "text=/New meeting|Start an instant meeting|Your meetings/i",
    {
      timeout: 0,
    },
  );
  ...
  await browser.close();
  console.log(
    "Finished. You can now run the bot containers using this auth.json.",
  );
})().catch((err) => {
